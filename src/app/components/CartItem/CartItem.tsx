...
function CartItem({ cartItem }: CartItemPropsType) {
  ...
  if (!context) return;
  const { increment, decrement, removeCartItem, shoppingCartItems, setShoppingCartItems } = context;
        ...
        <div className="w-full flex flex-row items-center md:justify-between  py-2 ">
          ...
          <ul className="w-full flex flex-row items-center justify-around md:justify-between  md:w-[50%] font-bold text-xs md:text-sm text-[var(--text-gray)]">
              ...
              <button
                ...
                onClick={() => {
                  if(cartItem?.count !== undefined && cartItem.count > 1) {
                    ...
                    setShoppingCartItems(shoppingCartItems.filter((el) => el.id !== cartItem.id))
                  }
                }}
            ...
            <li className="">
              ...
              ${" "}
              {cartItem?.count
                ? cartItem.sale ? cartItem.sale * cartItem?.count  : cartItem.price * cartItem?.count 
                : cartItem.price}
            </li>
          </ul>
