...
import React, { useContext, useEffect, useState } from "react";
import CheckIcon from "../_atoms/CheckIcon/CheckIcon";
import { ClobalContext } from "@/app/context/Context"; // Adjust the import as per your context file
import { useRouter } from "next/navigation";
...
const OrderSection = () => {
  ...
  useEffect(() => {
    if (context && shoppingCartItems && info) {
      const getInvoice = () => {
        const arr: ArrType[] = shoppingCartItems.map((item) => ({
          title: item.title,
          count: item.count ?? 1,
        ...
        const newObj = {
          ...info,
          purchase: arr,
          total: total ?  total <= 50 ? (total + 3.99).toFixed(2) : total.toFixed(2) : null,
          id: generateId(),
        };
  ...
  }, [context, shoppingCartItems, info, total]);
      ...
      <div className="h-full w-full flex flex-col  md:max-w-[500px] md:flex-row items-start rounded-xl overflow-hidden bg-slate-200 shadow-xl">
        <div className="w-full p-4 md:min-w-[250px] flex flex-col gap-6 ">
          ...
          <button
            onClick={() => {
              setShow(!show);
              setArrLength(show ? 1 : shoppingCartItems?.length || 1);
            }}
            className="text-xs flex items-center justify-center"
          >
            {arrLength === 1
              ? `and ${
                  shoppingCartItems ? shoppingCartItems.length - 1 : 0
                } other item(s)`
              : "View less"}
          </button>
        </div>
